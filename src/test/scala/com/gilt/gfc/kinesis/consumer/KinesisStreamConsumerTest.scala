...
class KinesisStreamConsumerTest extends FlatSpec with Matchers with MockitoSugar {
  ...
  "A KinesisStreamConsumer" should "pass batch of records to the processBatch function" in {
    val config = mock[KinesisClientConfiguration]
    ...
    val iut = new KinesisStreamConsumer("teststream", config)(processBatch)
  ...
  "A KinesisStreamConsumer configured to receive ALL empty batches" should "pass all empty batchs to the processBatch function" in {
    val config = mock[KinesisClientConfiguration]
  ...
  "a KinesisStreamConsumer" should "call onShutdown when a shard terminates" in {
    val config = mock[KinesisClientConfiguration]
    val processBatch = mock[Function2[Seq[Record], Checkpoint, Unit]]
    val kinesisCheckpointer = mock[IRecordProcessorCheckpointer]
    val onShutdown = mock[Function2[Checkpoint, String, Unit]]
    ...
    verify(onShutdown).apply(argThat(new CheckpointSizeMatcher(0)), mockEq("testshard"))
  }
  ...
  it should "call onShutdown when a shard terminates, passing the correct count of uncheckpointed records in the Checkpoint" in {
    ...
    verify(onShutdown).apply(argThat(new CheckpointSizeMatcher(2)), mockEq("testshard"))
  }
}
